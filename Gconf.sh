...
echo "Configuring nftables to wait for network-online.target..."
mkdir -p /etc/systemd/system/nftables.service.d
tee /etc/systemd/system/nftables.service.d/wait-for-network.conf > /dev/null <<EOF
[Unit]
After=network-online.target
Wants=network-online.target
EOF
...
echo "Restarting dnsmasq to ensure final settings apply..."
systemctl restart dnsmasq
